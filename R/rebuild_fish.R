...
rebuild_fish<-function(path_to_folder){
  ...
  # column ORDER refers to column letter (ABCDEF) in time series
  ...
  for i in (1:nrow(ds)){
    # Is there a filename listed in Codelist_id?
    if(!is.na(ds$Codelist_id[i])){
      ...
      merge_col<-tolower(ds$Codelist_Code_id[i]) # do the same to merge_col so it matches with cl column name
      #time_series_join<-left_join(time_series, cl_i, by = c((names(time_series)[i]) = merge_col))
      time_series_join<-left_join(time_series, cl_i, by = c("COUNTRY" = "un_code")) 
